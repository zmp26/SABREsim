...
// 	}
	...
	det3mc::det3mc(SABRE_Array* array,
			   std::vector<SABRE_EnergyResolutionModel*>& SABREARRAY_EnergyResolutionModels,
			   TargetEnergyLoss* targetLoss_par1,
			   TargetEnergyLoss* targetLoss_par2,
			   TargetEnergyLoss* targetLoss_par3,
...
// void det3mc::Run(std::ifstream& infile, std::ofstream& outfile, RootWriter* RootWriter, plot3mc* RootPlotter, SimConfig* config){
...
// 	// const auto& bu2 = config->GetBreakup2();
...
// 			//std::pair<int,int> hit1_rw = SABRE_Array_->at(i)->GetTrajectoryRingWedge(theta1*DEGRAD,phi1*DEGRAD);
...
// 			//std::pair<int,int> hit3_rw = SABRE_Array_->at(i)->GetTrajectoryRingWedge(theta3*DEGRAD,phi3*DEGRAD);
...
// 			//std::pair<int,int> hit4_rw = SABRE_Array_->at(i)->GetTrajectoryRingWedge(theta4*DEGRAD,phi4*DEGRAD);
...
void det3mc::Run(std::ifstream& infile, std::ofstream& outfile, EventRecorder* EventRecorder, plot3mc* RootPlotter, SimConfig* config){
	double e1, theta1, phi1, e2, theta2, phi2, e3, theta3, phi3, e4, theta4, phi4;
	...
	while(infile >> e1 >> theta1 >> phi1 >> e2 >> theta2 >> phi2 >> e3 >> theta3 >> phi3 >> e4 >> theta4 >> phi4){
		...
		std::ostringstream ss;
		ss << e1 << "\t" << theta1 << "\t" << phi1 << "\t" << e2 << "\t" << theta2 << "\t" << phi2 << "\t" << e3 << "\t" << theta3 << "\t" << phi3 << "\t" << e4 << "\t" << theta4 << "\t" << phi4;

		if(nevents_%50000==0) ConsoleColorizer::PrintBlue("Processed " + std::to_string(nevents_) + " events...\n");

		EventRecorder->SetKinematics(0,e1,theta1,phi1);
		EventRecorder->SetKinematics(0,e2,theta2,phi2);
		EventRecorder->SetKinematics(0,e3,theta3,phi3);
		EventRecorder->SetKinematics(0,e4,theta4,phi4);
		...
		for(size_t i=0; i<SABRE_Array_->size(); i++){
			...
			if(hit1_rw.first != -1 && hit1_rw.second != -1 && !detected1){
				...
				if(SABREARRAY_EnergyResolutionModels_[i]->detectEnergyInRing(hit1_rw.first,e1_afterDeadLayer,smearedERing1) && SABREARRAY_EnergyResolutionModels_[i]->detectEnergyInWedge(hit1_rw.second,e1_afterDeadLayer,smearedEWedge1)){
					...
					if(anglepair_optional){
						...
						theta1_meas = anglepair.first;
						phi1_meas = anglepair.second;
					}
			...
			if(hit3_rw.first != -1 && hit3_rw.second != -1 && !detected3){
				...
				if(SABREARRAY_EnergyResolutionModels_[i]->detectEnergyInRing(hit3_rw.first,e3_afterDeadLayer,smearedERing3) && SABREARRAY_EnergyResolutionModels_[i]->detectEnergyInWedge(hit3_rw.second,e3_afterDeadLayer,smearedEWedge3)){
					...
					if(anglepair_optional){
						...
						theta3_meas = anglepair.first;
						phi3_meas = anglepair.second;
					}
			...
			if(hit4_rw.first != -1 && hit4_rw.second != -1 && !detected4){
				...
				if(SABREARRAY_EnergyResolutionModels_[i]->detectEnergyInRing(hit4_rw.first,e4_afterDeadLayer,smearedERing4) && SABREARRAY_EnergyResolutionModels_[i]->detectEnergyInWedge(hit4_rw.second,e4_afterDeadLayer,smearedEWedge4)){
					...
					if(anglepair_optional){
						std::pair<double,double> anglepair = *anglepair_optional;
						Hit h = {34, (int)i, offsets[i].first + hit4_rw.first, offsets[i].second + hit4_rw.second, hit4_rw.first, hit4_rw.second, smearedERing4, smearedEWedge4, anglepair.first, anglepair.second, localCoords.GetX(), localCoords.GetY()};
						EventRecorder->AddHit(h);

						theta4_meas = anglepair.first;
						phi4_meas = anglepair.second;
					}
				}
			}
