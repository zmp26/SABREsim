...
plot4mc::plot4mc(const std::string& outname){
...
void plot4mc::FillKinematicsHistos(PHYSDATA& pd1, PHYSDATA& pd2, PHYSDATA& pd3, PHYSDATA& pd4){
	...
	histoman->getHisto1D("hBreakupLabAngle")->Fill(lab1.Angle(lab2)/DEGRAD);//check this!
}

void plot4mc::FillBeamSpotHisto(Vec3& reactionOrigin){
	histoman->getHisto2D("hBeamSpot")->Fill(reactionOrigin.GetX(), reactionOrigin.GetY());
}

void plot4mc::FillSABREHistos(SABREDATA& sd1, PHYSDATA& pd1){
...
bool plot4mc::FillTH2D(const TString& histoname, double valuex, double valuey){
	if(!histoman->getHisto2D(histoname)) return false;
	histoman->getHisto2D(histoname)->Fill(valuex, valuey);
	return true;
}

bool plot4mc::FillTH3D(const TString& histoname, double valuex, double valuey, double valuez){
	if(!histoman->getHisto3D(histoname)) return false;
	histoman->getHisto3D(histoname)->Fill(valuex, valuey, valuez);
	return true;
}

bool plot4mc::ParsePhysData(const std::string& line, PHYSDATA& pd1, PHYSDATA& pd2, PHYSDATA& pd3, PHYSDATA& pd4){
...
bool plot4mc::ParseSABREData(const std::string& line, SABREDATA& sd){
	std::istringstream iss(line);
...
void plot4mc::ProcessTXTOutput(std::vector<std::string> txtoutput){
	...
	for(const auto& line : txtoutput){
		if(line == "-1"){
			...
			if(eventLines.size() == 1){
				...
				//kinematics line plus 2 particle in SABRE
				ParsePhysData(eventLines[0], pd1, pd2, pd3, pd4);
				FillKinematicsHistos(pd1, pd2, pd3, pd4);
				ParseSABREData(eventLines[1],sd1);
				sd1.theta = sabre_thetaphimap[{sd1.ring+offsets[sd1.detectorIndex].first, sd1.wedge+offsets[sd1.detectorIndex].second}].first;//wow this is ugly but it works
				...
				sd3.theta = sabre_thetaphimap[{sd3.ring+offsets[sd3.detectorIndex].first, sd3.wedge+offsets[sd3.detectorIndex].second}].first;//wow this is ugly but it works
				sd3.phi = sabre_thetaphimap[{sd3.ring+offsets[sd3.detectorIndex].first, sd3.wedge+offsets[sd3.detectorIndex].second}].second;//likewise^
				ParseSABREData(eventLines[4],sd4);
				...
				sd4.phi = sabre_thetaphimap[{sd4.ring+offsets[sd4.detectorIndex].first, sd4.wedge+offsets[sd4.detectorIndex].second}].second;//likewise^
				...
				if(sd4.particleIndex == 100){
					FillSABREHistos(sd4, pd1);
				} else if(sd4.particleIndex == 200){
					FillSABREHistos(sd4, pd2);
				} else if(sd4.particleIndex == 300){
					FillSABREHistos(sd4, pd3);
				} else if(sd4.particleIndex == 400){
					FillSABREHistos(sd4, pd4);
				}
...
void plot4mc::ProcessTXTOutput(const std::string& outputLines){
...
double plot4mc::getSPSEnergy(double kinEnMeV, double sigmaMeV){
	double smearedE;
...
double plot4mc::calculateSPS_ExE(double spsE, double spsTheta, double spsPhi){//we should rethink this...hard coded for 6Li but also only used to fill explicitly 6Li ExE histos...
...
void plot4mc::readSingleAngleMap(std::ifstream& infile, std::map<std::pair<int,int>,std::pair<double,double>>& map){
	std::string header;
