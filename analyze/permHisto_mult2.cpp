...
permHisto_mult2::permHisto_mult2(TString permName, TDirectory* targetDir){
	...
	for(const auto& p : particles){
		...
		Register2D(permName, p+"vcm_TransverseVSLongitudinal", p+" Velocity CM Transverse vs Longitudinal;c;c", 1000, 0, 0.1, 1000, 0, 0.1);
	...
	Register2D(permName, "ecmmeasVSfrag1thetacm", "E_{cm} Decay (meas) vs frag1 ThetaCM;MeV;deg", 360, 0, 180, 600, -1, 5);
	Register2D(permName, "ecmmeasVSfrag2thetacm", "E_{cm} Decay (meas) vs frag2 ThetaCM;MeV;deg", 360, 0, 180, 600, -1, 5);
    
	Register1D(permName, "decay_VCM", "VCM Decay;c", 1000, 0, 0.1);
	Register2D(permName, "decay_VCM_TransverseVSLongitudinal", "Decay VCM Transverse Vs Longitudinal", 1000, 0, 0.10, 1000, 0, 0.10);
	Register1D(permName, "decay_thetaCMsum", "Decay Theta CM Sum", 720, 0, 360);
	Register1D(permName, "decay_phiCMdiff", "Decay Phi CM Diff", 720, 0, 360);
