...
permHistoMM_mult2::permHistoMM_mult2(TString permName, TDirectory* targetDir){
	...
	Register2D(permName, "RecoilEx_IMvsSPS", "Recoil Ex IM vs SPS;SPS MeV;IM MeV",300,-5,7,300,-5,7);
	Register1D(permName, "RecoilExDif", "Recoil Ex (SPS - IM);MeV", 100, -2, 2);
	Register2D(permName, "intermediateExIMvsSPS", "intermediate Ex IM vs SPS;SPS MeV;IM MeV",300,-5,7,300,-5,7);
	...
	for(const auto& p : particles){
		...
		Register1D(permName, p+"thetacm", p+" Theta CM; deg", 360, 0, 180);
		Register1D(permName, p+"phicm", p+" Phi CM; deg", 720, 0, 360);
		Register2D(permName, p+"thetacmvsphicm", p+" ThetaCM vs PhiCM;deg;deg", 720, 0, 360, 360, 0, 180);

		Register2D(permName, p+"vcmVSthetacm", p+" V CM vs Theta CM", 360, 0, 180, 1000, 0, 0.1);
		Register2D(permName, p+"kecmVSthetacm", p+" KE CM vs Theta CM", 360, 0, 180, 500, 0, 5);
	}

	//missing mass specific histograms (note that these will change meaning between permutations!)
	Register1D(permName, "MissingMass", "Reconstructed Missing Mass;MeV/c^{2}", 5000, 0, 10000);
	Register2D(permName, "intermediateExVSMissingMass", "intermediate vs ExMissing Mass;MeV;MeV/c^{2}", 5000, 0, 10000, 300, -5, 7);
	...
	Register2D(permName, "ecm1measVSintermediatethetacm", "E_{cm} Decay 1 (meas) vs Intermediate ThetaCM;MeV;deg", 360, 0, 180, 600, -1, 5);
	Register2D(permName, "ecm1measVSfrag1thetacm", "E_{cm} Decay 1 (meas) vs frag1 ThetaCM;MeV;deg", 360, 0, 180, 600, -1, 5);
	...
	Register2D(permName, "ecm2measVSintermediatethetacm", "E_{cm} Decay 2 (meas) vs Intermediate ThetaCM;MeV;deg", 360, 0, 180, 600, -1, 5);
	Register2D(permName, "ecm2measVSfrag1thetacm", "E_{cm} Decay 2 (meas) vs frag1 ThetaCM;MeV;deg", 360, 0, 180, 600, -1, 5);
