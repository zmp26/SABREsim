...
void SABRE_ParticleID_Splitter_N2(const char* infile, const char* intree = "SABREsim"){
	...
	//unique name sfor isolated combinations: B->c+d, if detect only one must be c or d
	std::string outC_name = MakePairOutputName(infile, "mult1_c");//missing d
	std::string outD_name = MakePairOutputName(infile, "mult1_d");//missing c

	TFile *fC = new TFile(outC_name.c_str(), "RECREATE");
	TTree *tC = new TTree("mult1", "Detected C (Missing D)");

	TFile *fD = new TFile(outD_name.c_str(), "RECREATE");
	TTree *tD = new TTree("mult1", "Detected D (Missing C)");
	...
	auto BindBranches = [&](TTree* t){
		...
		t->Branch("thetalab_hit1", &Stheta1, "thetalab_hit1/D");
		t->Branch("philab_hit1", &Sphi1, "philab_hit1/D");
		t->Branch("key_hit1", &key, "key_hit1/I");
	};
	...
	for(Long64_t i=0; i<nentries; i++){
		tin->GetEntry(i);
		if(tin_multiplicity != 1) continue;
		...
		//direct sorting based on the natural loop output sequence of det3mc
		if(id == 3) tC->Fill();
		else if(id == 4) tD->Fill();
	}

	fC->cd(); tC->Write(); fC->Close(); delete fC;
	fD->cd(); tD->Write(); fD->Close(); delete fD;
